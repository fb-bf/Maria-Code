...
#include <esp_wifi.h>
//#include <Wire.h>
// Boom Angle limit switch vales are 15 for 2 hour timer. 16 and 17 for eiher limit switch activation.
...
void print_wakeup_reason(){
  ...
  switch(wakeup_reason)
  {
    ...
    if(log(GPIO_reason)/log(2) == 32){
      Trigger_state.Trigger = 16; //We'll send the value 13 if this is caused by the limit switch
    }
    if(log(GPIO_reason)/log(2) == 27){
     Trigger_state.Trigger = 17; //We'll send the value 14 if this is caused by the other limit switch
    }   
    ...
    Trigger_state.Trigger = 15; //We'll send the value 12 if this is caused by the Timer
    break;
