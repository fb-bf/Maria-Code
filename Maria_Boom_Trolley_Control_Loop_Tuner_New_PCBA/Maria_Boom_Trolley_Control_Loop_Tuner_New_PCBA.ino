...
void TrolleyControl(){
  ...
  TrolleyError = trolleyLocation * Trencoderperft - trolleyCurrentPosition;
...
void DragflowControl(){
  ...
  // dragflowvelocity is positive when the winch is moving out
   ...
   if(!Linkwinches){
     dragflowPosition = dragflowheight * Dfencoderperft; // We'll use feet for controlling the height
   }
   else{
     dragflowPosition = (trolleyCurrentPosition-trolleytransitionPosition) - dragflowheight*Dfencoderperft;
   }
...
/*String Form() {
 ...
 String TrolleyForm() {
  ...
  Value += ("<form action='/form/submit' method='get' <Label> </Label>");
  //Value += ("<Label> </Label>");
      Value += ("New_Trolley_Location:<input type='text' value="+String(trolleyLocation)+" name='New_Trolley_Location'/>");
      Value += ("Gain Value:<input type='text' value="+String(buffer1)+" name='New_Gain'/>");
      Value += ("Diff Value:<input type='text' value="+String(buffer2)+" name='New_Diff'/>");
