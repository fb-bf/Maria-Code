...
#include <esp_wifi.h>
//#include <Wire.h>
// Dragflow limit switch vales are 21 for 2 hour timer. 22 and 23 for eiher limit switch activation.
...
void print_wakeup_reason(){
  ...
  switch(wakeup_reason)
  {
    ...
    if(log(GPIO_reason)/log(2) == 32){
      Trigger_state.Trigger = 22; //We'll send the value 13 if this is caused by the limit switch
    }
    if(log(GPIO_reason)/log(2) == 27){
     Trigger_state.Trigger = 23; //We'll send the value 14 if this is caused by the other limit switch
    }   
    ...
    Trigger_state.Trigger = 21; //We'll send the value 12 if this is caused by the Timer
    break;
