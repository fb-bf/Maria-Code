...
byte sluice_on1 = 15; //after the first minute of the new 16 minute rinse cycle
// rotate sluice for 1 minute;
byte sluice_off1 = 14; // turn off the sluice for next 7 minutes
byte sluice_on2 = 7;  // used for latest version of 16 min cycle
byte sluice_off2 = 6; //  used for latest version of 16 min cycle
byte sluice_on3 = 2;  // not used for latest version of 16 min cycle
byte sluice_off3 = 1; // not used
...
void setup() {
  ...
  Sequencer_Time = 275; // little greater than 16 minutes 1024*16/60
  // *************************************
 ...
 // WiFi.mode(WIFI_AP_STA);
  ...
  if (esp_now_init() != ESP_OK) {
    ...
    lcd.setCursor(0, 0); // Line 1, space 1
      //lcd.print("cycletype "+String(Cycle_Type) );
  ...
  //if (esp_now_add_peer(&peerInfo) != ESP_OK){
    ...
    lcd.setCursor(0, 1); // Line 1, space 1
    
    //return;
    esp_err_t addStatus = esp_now_add_peer(&peerInfo);
    if (addStatus == ESP_OK) {
      // Pair success
      lcd.print("Pair success");
    } else if (addStatus == ESP_ERR_ESPNOW_NOT_INIT) {
      // How did we get so far!!
      lcd.print("ESPNOW Not Init");
    } else if (addStatus == ESP_ERR_ESPNOW_ARG) {
      lcd.print("Invalid Argument");
    } else if (addStatus == ESP_ERR_ESPNOW_FULL) {
      lcd.print("Peer list full");
    } else if (addStatus == ESP_ERR_ESPNOW_NO_MEM) {
      lcd.print("Out of memory");
    } else if (addStatus == ESP_ERR_ESPNOW_EXIST) {
      lcd.print("Peer Exists");
    } else {
      lcd.print("Not sure ");
    }
  ...
  //if (esp_now_add_peer(&peerInfo) != ESP_OK){
    ...
    lcd.setCursor(0, 2); // Line 1, space 1
    //lcd.print("VFD1 not seen" );
    //delay(500);
    //VFD1_Problem = 1;
    //return;
  ...
  //if (esp_now_add_peer(&peerInfo) != ESP_OK){
    ...
    lcd.setCursor(0, 3); // Line 1, space 1
    //lcd.print("VFD2 not seen" );
    //delay(5000);
    //VFD2_Problem = 1;
    //return;
    addStatus = esp_now_add_peer(&peerInfo);
  ...
  if (result == ESP_OK) {
    ...
    lcd.clear();
    lcd.setCursor(0, 0); // Line 1, space 1
    lcd.print("Valve Didn't Respond" );
    delay(5000);    
  }
...
int Cycle_Type = 0;
   ...
   case Plant_stopped:
      ...
      lcd.setCursor(0, 0); // Line 1, space 1
      //lcd.print("cycletype "+String(Cycle_Type) );
      lcd.print("Waiting 2 Initialize" );
 ...
 //Deal_With_client();
  ...
  lcd.clear();
  //lcd.print(" " + String(Time_in_Minutes) +":"+ String(Time_in_Seconds) + " Till Rinse");
  lcd.print(" " + RinseTime + " Till Rinse ");
  lcd.setCursor(0, 3);
  ...
  if (Conveyor_sluice_manual_off == 0){ 
    ...
    if (TM == sluice_on1 || TM == sluice_on2 ){
      VFD_Conveyor_Sluice = 1;
      lcd.setCursor(0, 2);
      lcd.print(" Conveyor Sluice on ");
      Update_VFD2_Module();
    }
  }
...
void Begin_Rinse_Cycle(int Count) {
  ...
  lcd.setCursor(0, 0);
  lcd.print("   Rinsing Plant   ");
  lcd.setCursor(0, 3); // Line 3, space 1
  lcd.print("cycletype "+String(Cycle_Type) );
    switch (Cycle_Type) {
      case First_Cycle:
...
void Stop_Plant_And_Rinse_All_Concentrators() {
 // Stop Feed hopper
   ...
   lcd.clear();
   lcd.setCursor(0, 0);
   lcd.print("  Stopping Plant ");  
   
 
 VFD_Hopper = 0;  // Turn off
 Update_VFD2_Module();
 lcd.setCursor(0, 1);
 lcd.print("   Hopper off"); //
 lcd.setCursor(0, 2);
 lcd.print(" Waiting 20 seconds ");
 delay(20000); //wait 20 seconds
 
 VFD_60_ft_Conveyor = 0; // Turn off
 Update_VFD2_Module();
 lcd.setCursor(0, 1);
 lcd.print("60ft Conveyor off"); //
 lcd.setCursor(0, 2);
 lcd.print(" Waiting 10 seconds ");
 ...
 /*lcd2.setCursor(0,0);
 lcd2.print("  Screen off"); //
 lcd2.setCursor(0, 1);
 lcd2.print(" Tails Conveyor off "); //
 lcd2.setCursor(0,2);
 lcd2.print("  Conveyor Sluice "); //
 lcd2.setCursor(0, 3);
 lcd2.print(" Set to high Speed "); */
...
// New delay added on the running the conveyor sluice DAD
 ...
 // open all pinch valves
  ...
  lcd.setCursor(0,1);
  lcd.print(" B Rinse cycle"); // 
  lcd.setCursor(0,2);
  lcd.print(" Running"); // 
  Begin_Rinse_Cycle(4); //Do the "B" Rinse cycle
  lcd.setCursor(0,1);
  lcd.print(" C Rinse cycle"); // 
  lcd.setCursor(0,2);
  lcd.print(" Running"); // 
...
void Update_Valve_Module(int Cycle_Type){
  ...
  //  4 for A secondpass, 5 for B secondpass, 6 for C secondpass, 7 for Normal operation
   ...
   switch (Cycle_Type) {
    ...
    Data_1.valve_relay_states &= 0b1011111111111111; Data_1.valve_relay_states |= Conc1_Air_Valve<<14;
    Data_1.valve_relay_states &= 0b0111111111111111; Data_1.valve_relay_states |= Conc2_Air_Valve<<15;
    Data_1.valve_relay_states &= 0b1111111111111110; Data_1.valve_relay_states |= Conc3_Air_Valve;
    Data_1.valve_relay_states &= 0b1111111111111101; Data_1.valve_relay_states |= Conc4_Air_Valve<<1;
    Data_1.valve_relay_states &= 0b1111111111111011; Data_1.valve_relay_states |= Conc5_Air_Valve<<2;
    Data_1.valve_relay_states &= 0b1111111111110111; Data_1.valve_relay_states |= Conc6_Air_Valve<<3;
...
void Update_VFD1_Module(int Cycle_Type){
  ...
  esp_err_t result2 = esp_now_send(broadcastAddress2, (uint8_t *) &Data_2, sizeof(VFD_Contoller));
  if (result2 == ESP_OK) {
    Serial.println("Sent with success 0x02");
  }
  else {
    Serial.println("Error sending the data");
  }
  delay(100);
  while( VFD1_Problem == 1) {
    Serial.print("VFD1_receive_error");
    VFD1_retry_counter++;
    if (VFD1_retry_counter > 5){
      VFD1_retry_counter = 0;
      lcd.setCursor(0, 2); // Line 3, space 1
      lcd.print("   VFD1_Problem " );
      return;
    }
    esp_err_t result2 = esp_now_send(broadcastAddress2, (uint8_t *) &Data_2, sizeof(VFD_Contoller));
    delay(10);
...
void Update_VFD2_Module(){
  ...
  esp_err_t result3 = esp_now_send(broadcastAddress3, (uint8_t *) &Data_3, sizeof(VFD_Contoller2));
  if (result3 == ESP_OK) {
    Serial.println("Sent with success 0x03");
  }
  else {
    Serial.println("Error sending the data");
  }
  delay(100);
  while( VFD2_Problem == 1) {
    Serial.println("VFD2_receive_error");
    VFD2_retry_counter++;
    if (VFD2_retry_counter > 5){
      VFD2_retry_counter = 0;
      lcd.setCursor(0, 3); // Line 4, space 1
      lcd.print("   VFD2_Problem " );
      return;
    }
    esp_err_t result3 = esp_now_send(broadcastAddress3, (uint8_t *) &Data_3, sizeof(VFD_Contoller2));
    delay(10);
