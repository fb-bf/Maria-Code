...
void ControlLoop() {  
 if (RunLoop){ // Checks to see if the loop is disabled 
    ...
    HoseReelOverride(); // This checks to see if the hose reel is keeping the proper amount of hose out
     ...
     boomServoOutput = 2048;
     dragflowServoOutput = 2048;
     trolleyServoOutput = 2048;
     hoseReelServoOutput = 2048;
     hoseGuideServoOutput = 2048;     
    }
...
void SweepControl(){
   if(SweepOn) {
     ...
     if(SweepDirectionPositive && boomPosition > sweepfinishangle*Bencoderperdeg){
       SweepOn = false;
...
void BoomControl(){
  ...
  // when we're dealing with PWM width.
   ...
   boomServoOutput = 2048 - boomControlCommand;
   }
   else{
    boomServoOutput = 2048;
   }
...
void TrolleyControl(){
  ...
  // Trolley error is defined as positive when desired position is farther out than current position
   trolleyCurrentPosition = (int32_t)trolleyencoder.getCount();
     ...
     trolleyServoOutput = 2048 - trolleyControlCommand;
     }
     else{
        trolleyServoOutput = 2048; 
     }
...
void DragflowControl(){
  ...
  // dragflowvelocity is positive when the winch is moving out
   ...
   dragflowServoOutput = 2048 - dragflowControlCommand;
   }
   else{
     dragflowServoOutput = 2048; 
   }
...
void HoseReelControl(){
  ...
  // HoseReelError is defined as positive if hoseReelPosition is farther out than Current position
   ...
   hoseReelServoOutput = 2048 - hoseReelControlCommand;
   }
   else{
     hoseReelServoOutput = 2048; 
   }   
...
void HoseGuideControl(){
  ...
  // This is about 5.3 degrees (7073 encoder counts) scale is then .0245
   ...
   hoseGuideServoOutput = 2048 - hoseGuideControlCommand;
   }
   else{
     hoseGuideServoOutput = 2048; 
   }   
...
void create_json_success_or_fail( bool value) { 
  jsonDocument.clear();  
  jsonDocument["Success"] = value;
...
void handlePoststop(){
  ...
  if (!Stop) dragflowPosition = dragflowCurrentPosition;
...
void handlePostParams(){
  ...
  jsonDocument.clear(); 
  if (Foundkey)jsonDocument["Status"] = "Succes";
