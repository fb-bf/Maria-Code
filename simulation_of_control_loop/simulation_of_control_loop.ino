...
void MoveControl(){
  // NOTE: We could maybe update the state inside the actual functions doing the work to avoid potential invalid states
  switch (MoveState){
    case NotMoving:
      break;
    case StartMove:
...
int32_t BOOM_SIMULATED_POSITION = 0;
void BoomControl(){
  //LoopCounter++;
  ...
  // when we're dealing with PWM width.
   ...
   boomCurrentPosition = BOOM_SIMULATED_POSITION;
   ...
   if(abs(BoomError) > Boomdb){ 
     ...
     if(boomControlCommand > BoomClipRight) boomControlCommand = BOOM_SIMULATED_POSITION--;
     if(boomControlCommand < BoomClipLeft) boomControlCommand = BOOM_SIMULATED_POSITION++;
   }
   
   boomLastPosition = boomCurrentPosition;   
}

int32_t TROLLEY_SIMULATED_POSITION = 0;
void TrolleyControl(){
  ...
  // Trolley error is defined as positive when desired position is farther out than current position
   trolleyCurrentPosition = TROLLEY_SIMULATED_POSITION;
   ...
   if(abs(TrolleyError) > Trolleydb){
    ...
    if(trolleyControlCommand > TrolleyClipIn) TROLLEY_SIMULATED_POSITION--;
    // negative control efforts means we're pushing the trolley out
    if(trolleyControlCommand < TrolleyClipOut) TROLLEY_SIMULATED_POSITION++;
...
int32_t DRAGFLOW_SIMULATED_POSITION = 0;
void DragflowControl(){
  ...
  // dragflowvelocity is positive when the winch is moving out
   ...
   dragflowCurrentPosition = DRAGFLOW_SIMULATED_POSITION;
   ...
   if(abs(DragflowError) > Dragflowdb){
    ...
    if(dragflowControlCommand > DragflowClipUp) DRAGFLOW_SIMULATED_POSITION--;
    if(dragflowControlCommand < DragflowClipDown) DRAGFLOW_SIMULATED_POSITION++;
   }
   
   dragflowLastPosition = dragflowCurrentPosition;
}

int32_t HOSEREEL_SIMULATED_POSITION = 0;
void HoseReelControl(){
  ...
  // HoseReelError is defined as positive if hoseReelPosition is farther out than Current position
   ...
   hoseReelCurrentPosition = HOSEREEL_SIMULATED_POSITION;
   ...
   if(abs(HoseReelError) > HoseReeldb){
    ...
    if(hoseReelControlCommand > HoseReelClipOut) HOSEREEL_SIMULATED_POSITION--;
    if(hoseReelControlCommand < HoseReelClipIn) HOSEREEL_SIMULATED_POSITION++; 
   }   
...
int32_t HOSEGUIDE_SIMULATED_POSITION = 0;
void HoseGuideControl(){
  ...
  // This is about 5.3 degrees (7073 encoder counts) scale is then .0245
   ...
   hoseGuideCurrentPosition = HOSEGUIDE_SIMULATED_POSITION;
