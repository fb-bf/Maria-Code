...
#define BUTTON_PIN_BITMASK 0x10A000000 // 2^32 in hex and 2^27 and 2^25 use Pin 32 for one limit switch, pin 27 for the other
...
void print_wakeup_reason(){
  ...
  switch(wakeup_reason)
  {
    ...
    if(log(GPIO_reason)/log(2) == 32){
      Trigger_state.Trigger = 13; //We'll send the value 13 if this is caused by the limit switch
    }
    if(log(GPIO_reason)/log(2) == 27){
     Trigger_state.Trigger = 14; //We'll send the value 14 if this is caused by the other limit switch
    }   
    ...
    Trigger_state.Trigger = 12; //We'll send the value 12 if this is caused by the Timer
    break;
