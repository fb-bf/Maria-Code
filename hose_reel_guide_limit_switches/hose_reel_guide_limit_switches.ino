...
#include <esp_wifi.h>
//#include <Wire.h>
// Hose reel Guid limit switch vales are 9 for 2 hour timer. 10 and 11 for eiher limit switch activation.
...
#define BUTTON_PIN_BITMASK 0x108000000 // 2^32 in hex and 2^27 use Pin 32 for one limit switch, pin 27 for the other
                                  //  for limit switch and enable pin to light LED. All 3 wake up from sleep
...
void print_wakeup_reason(){
  ...
  switch(wakeup_reason)
  {
    ...
    if(log(GPIO_reason)/log(2) == 32){
      Trigger_state.Trigger = 10; //We'll send the value 13 if this is caused by the limit switch
    }
    if(log(GPIO_reason)/log(2) == 27){
     Trigger_state.Trigger = 11; //We'll send the value 14 if this is caused by the other limit switch
    }   
    ...
    Trigger_state.Trigger = 9; //We'll send the value 4 if this is caused by the Timer
    break;
