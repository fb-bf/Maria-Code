...
float PotScale;
int MyCounter = 0;
byte Page = 1; //used to tell us what page we want diplayed
...
int32_t trolleytransitionPosition = 0;
float trolleyLocationft = 0;
int trolleyServoOutput = 0;
int LasttrolleyServoOutput = 0;
float maxtrolleyLocationft = 90;
int32_t trolleyCurrentPosition = 0;
int32_t TrolleyError = 0;
float Trencoderperft = 15279.; // Using halfQuad Overwritten when Saved variables are read 
...
float Dfencoderperft = 15279.; // Overwritten when Saved variables are read
...
typedef struct Density_str {
  uint16_t Density;
  uint16_t GallonsPerMinute;
  uint16_t MassFlow;
} Density_str;
Density_str Density_Info;
...
bool HosereelManualControl = true;
bool SkipDragLoop = false;
bool SkipHosereelLoop = false;
...
void OnDataRecv(const uint8_t *mac,const uint8_t*incomingData, int len) {
  if (len > 1) {
    memcpy(&Density_Info, incomingData, sizeof(Density_Info));
    Serial.print("Bytes received: ");
    Serial.println(len);
    //Serial.print("Density Value ");
   // Serial.println(Density_Info.Density);
   // Serial.print("GPM Value ");
   // Serial.println(Density_Info.GallonsPerMinute);
    DensityData = 1;
  }else{
...
void BoomControl(){
  ...
  // when we're dealing with PWM width.
   ...
   int32_t BoomError = BoomLocationDeg * Bencoderperdeg - boomCurrentPosition;
...
void TrolleyControl(){
  ...
  TrolleyError = trolleyLocationft * Trencoderperft - trolleyCurrentPosition;
  ...
  trolleyLastPosition = trolleyCurrentPosition;
   ...
   dragflowPosition = trolleyCurrentPosition - dragflowheightft*Dfencoderperft;
   if(transition) {
   trolleytransitionPosition = trolleyCurrentPosition;
   transition = false; //Do this once
   }
   // dragflowheightft is defined as 0 when at home height, and negative when below home height
...
void DragflowControl(){
  ...
  // dragflowvelocity is positive when the winch is moving out
   ...
   dragflowCurrentPosition = (int32_t)dragflowencoder.getCount();
   if(!Linkwinches){
     dragflowPosition = dragflowheightft * Dfencoderperft; // We'll use feet for controlling the height
   }
   else{
     dragflowPosition = (trolleyCurrentPosition-trolleytransitionPosition) - dragflowheightft*Dfencoderperft;
   }
...
void HoseReelControl(){
  ...
  // HoseReelError is defined as positive if hoseReelPosition is farther out than Current position
   ...
   hoseReelCurrentPosition = (int32_t)hosereelencoder.getCount();
   int32_t HoseReelError = hoseReelPositionft * Hrencoderperft - hoseReelCurrentPosition;
...
void parseGainValues(){  // This is for finding the gain, and clipin and clipout
  int EqualSignLoc = 0;
  int SpaceFollowing = 0;
  String returnValue = "";
  const char * temp;
...
void parseMoveValues(){  // This is for finding the gain, and clipin and clipout
  int EqualSignLoc = 0;
  int SpaceFollowing = 0;
  String returnValue = "";
  const char * temp;
  ...
  returnValue = header.substring(EqualSignLoc+1,SpaceFollowing);
  temp = returnValue.c_str();
  moveValue = std::atof(temp);  
}
void parseEncoderValues(){  // This is for finding the gain, and clipin and clipout
  int EqualSignLoc = 0;
  int SpaceFollowing = 0;
  String returnValue = "";
  const char * temp;
...
void setup() {
  ...
  int32_t channel = 11;     //It looks like only WIFI_STA works
  //WiFi.softAP(ssid);
  ...
  pref.end();
  Trencoderperft = 15278.87; //changed for going to half quad
  Dfencoderperft = 15278.87; //changed for going to half quad
...
/*
 ...
 // Set up the encoders *******************************
  ...
  trolleyencoder.attachHalfQuad(35, 34);
  dragflowencoder.attachHalfQuad(33, 32);
...
void loop() {
  ...
  //if(DensityData == 1) might need something here
      ...
      if (client.available()) {             // if there's bytes to read from the client,
        ...
        if (c == '\n') {                    // if the byte is a newline character
          ...
          if (currentLine.length() == 0) {
            ...
            else if (header.indexOf("GET /Zero_Hosereel_Enc") >= 0){
              hosereelencoder.setCount(0);
            }
            ...
            else if (header.indexOf("GET /Main_Page") >= 0){ 
                Page = 1;           
            }
            else if (header.indexOf("GET /Boom_Page") >= 0){ 
                Page = 2;           
            }
            else if (header.indexOf("GET /Trolley_Page") >= 0){ 
                Page = 3;           
            }
            else if (header.indexOf("GET /Dragflow_Page") >= 0){ 
                Page = 4;           
            }
            else if (header.indexOf("GET /Hosereel_Page") >= 0){ 
                Page = 5;           
            }
            else if (header.indexOf("GET /Hoseguide_Page") >= 0){ 
                Page = 6;           
            }
            else if (header.indexOf("GET /Backpressure_control") >= 0){ 
                if (Pressureon == true){ //if this is true then we are already in backpressure on
                  ValveValue = 1000;  // change to manual mode
                  Pressureon = false;
                }else{ // IfpPressureon is false then we are already in Manual mode, swith to pressure on
                ValveValue = 4000;
                Pressureon = true;
                }
                dac3.setDACOutVoltage(ValveValue, 1);
            }
            else if (header.indexOf("GET /BoomControl") >= 0) {
            ...
            else if (header.indexOf("GET /TrolleyControl") >= 0) {
            ...
            else if (header.indexOf("GET /HosereelControl") >= 0) {
              if (HosereelManualControl == true) {
                dac1.setDACOutVoltage(500, 1); //set to below 15% of 10 volts to goto manual control
                SkipHosereelLoop = true;
                HosereelManualControl = false;
              } else {
                dac1.setDACOutVoltage(5000, 1); //set to home (5 volts)
                SkipHosereelLoop = false; //Turn boom loop back on
                HosereelManualControl = true;
              }
            }
            else if (header.indexOf("GET /DragControl") >= 0) {
            ...
            else if (header.indexOf("GET /form/submit?New_Gain") >= 0) {
              parseGainValues();
              if( Page == 2){
              BoomG = gainValue;
            ...
            else if (header.indexOf("GET /form/submit?Move_To") >= 0) {
              parseMoveValues();
              if( Page == 2){
                BoomLocationDeg = moveValue;
              }
              if( Page == 3){
                trolleyLocationft = moveValue;
              }
              if( Page == 4){
                dragflowheightft = moveValue;
              }
              if( Page == 5){
                hoseReelPositionft = moveValue;
              }
            }
            else if (header.indexOf("GET /form/submit?New_Encoder_Value") >= 0) {
              parseEncoderValues();
              if( Page == 2){
                Bencoderperdeg = encoderValue;
              }
              if( Page == 3){
                Trencoderperft = encoderValue;
              }
              if( Page == 4){
                Dfencoderperft = encoderValue;
              }
              if( Page == 5){
                Hrencoderperft = encoderValue;
              }
            }
...
// Display the HTML web page
            ...
            if(Page ==1){
              STYLE_Content = Stylecode();
              client.println(STYLE_Content);
              Main_Content = Main();
              client.println(Main_Content);
            }
            if(Page ==2){
              STYLE_Content = Stylecode();
              client.println(STYLE_Content);
...
/*String Form() {
 ...
 String BoomForm() {
  ...
  char buffer1[10];
  sprintf(buffer1,"%.1f",BoomG);
  char buffer2[10];
  //boomCurrentLocationDeg = boomCurrentPosition * Bencoderperdeg;
  sprintf(buffer2,"%.1f",(boomCurrentPosition * Bencoderperdeg)); 
  String Value ="<p>";
  if (BoomManualControl == true) {
    Value += "<a href=\"/BoomControl\"><button class=\"button btn_on\">Boom in Servo mode</button></a>";
  } else {
    Value += "<a href=\"/BoomControl\"><button class=\"button btn_off\">Boom in Manual mode</button></a>";
  }
  Value += ("<a href=\"/Zero_Boom_Enc\"><button class=\"button btn_on\">Zero Boom</button></a></p>");
  Value +=("<div>");

  Value += ("<form action='/form/submit' method='get' <Label> </Label>");
  Value += ("New_Gain:<input type='text' value="+String(buffer1)+" name='New_Gain'/>");
 ...
 String TrolleyForm() {
  ...
  //trolleyCurrentLocationft = trolleyCurrentPosition * Trencoderperft;
  sprintf(buffer2,"%.1f",(trolleyCurrentPosition * Trencoderperft)); // do this to get 3 decimal places displayed

  String Value ="<p>";
  if (TrolleyManualControl == true) {
    Value += "<a href=\"/TrolleyControl\"><button class=\"button btn_on\">Trolley in Servo mode</button></a>";
  } else {
    Value += "<a href=\"/TrolleyControl\"><button class=\"button btn_off\">Trolley in Manual mode</button></a>";
  }
  Value += ("<a href=\"/Zero_Trolley_Enc\"><button class=\"button btn_on\">Zero Trolley</button></a></p>");
  Value +=("<div>");
  Value += ("<form action='/form/submit' method='get' <Label> </Label>");
  Value += ("New_Gain:<input type='text' value="+String(buffer1)+" name='New_Gain'/>");
 ...
 String DragflowForm() {
  ...
  sprintf(buffer2,"%.1f",(dragflowCurrentPosition * Dfencoderperft)); 
  String Value ="<p>";
  if (DragManualControl == true) {
    Value += "<a href=\"/DragControl\"><button class=\"button btn_on\">Drag in Servo mode</button></a>";
  } else {
    Value += "<a href=\"/DragControl\"><button class=\"button btn_off\">Drag in Manual mode</button></a>";
  }
  Value += ("<a href=\"/Zero_Drag_Enc\"><button class=\"button btn_on\">Zero Dragflow</button></a></p>");
  Value +=("<div>");
  Value += ("<form action='/form/submit' method='get' <Label> </Label>");
  Value += ("New_Gain:<input type='text' value="+String(buffer1)+" name='New_Gain'/>");
 ...
 String HoseReelForm() {
  ...
  if (HosereelManualControl == true) {
    ...
    Value += "<a href=\"/HosereelControl\"><button class=\"button btn_off\">Hosereel in Manual mode</button></a>";
  }
  Value += ("<a href=\"/Zero_Hosereel_Enc\"><button class=\"button btn_on\">Zero Hosereel</button></a></p>");
...
String Main() {
  ...
  Value += ("<p><a href=\"/Save_To_EEprom\"><button class=\"button btn_on\">Save to EEprom</button></a>");
  if (BoomManualControl == true) {
    Value += "<a href=\"/BoomControl\"><button class=\"button btn_on\">Boom in Servo mode</button></a>";
  } else {
    Value += "<a href=\"/BoomControl\"><button class=\"button btn_off\">Boom in Manual mode</button></a>";
  ...
  if (DragManualControl == true) {
    Value += "<a href=\"/DragControl\"><button class=\"button btn_on\">Dragflow in Servo mode</button></a>";
  } else {
    Value += "<a href=\"/DragControl\"><button class=\"button btn_off\">Dragflow in Manual mode</button></a>";
  }
  if (HosereelManualControl == true) {
    Value += "<a href=\"/HosereelControl\"><button class=\"button btn_on\">Hosereel in Servo mode</button></a></p>";
  } else {
    Value += "<a href=\"/HosereelControl\"><button class=\"button btn_off\">Hosereel in Manual mode</button></a></p>";
  }
  if (Linkwinches == false) {
    Value += "<p><a href=\"/Link_Winches\"><button class=\"button btn_on\">Winches Unlinked</button></a>";
  } else {
    Value += "<p><a href=\"/Link_Winches\"><button class=\"button btn_on\">Winches linked</button></a>";
  }
  if (Pressureon == true) {
    Value += "<a href=\"/Backpressure_control\"><button class=\"button btn_on\">Backpressure is on</button></a>";
  } else {
    Value += "<a href=\"/Backpressure_control\"><button class=\"button btn_off\">Backpressure is in Manual Mode</button></a>";
  }
  //Value += "<a href=\"/Pressure_On\"><button class=\"button btn_on\">Backpressure on</button></a>";
  //Value += "<a href=\"/Pressure_Off\"><button class=\"button btn_off\">Backpressure Manual Mode</button></a>";
  /*if (RunLoop == false) {
    Value += "<p><a href=\"/Loop_Control\"><button class=\"button btn_on\">Loop Stopped</button></a></p>";
  } else {
    Value += "<p><a href=\"/Loop_Control\"><button class=\"button btn_on\">Loop Running</button></a></p>";
  }*/
  
   Value += ("<a href=\"/Boom_Page\"><button class=\"button btn_on\">Go to Boom settings</button></a>");
   Value += ("<a href=\"/Trolley_Page\"><button class=\"button btn_on\">Go to Trolley settings</button></a></p>");
   Value += ("<p><a href=\"/Dragflow_Page\"><button class=\"button btn_on\">Go to Dragflow settings</button></a>");
   Value += ("<a href=\"/Hosereel_Page\"><button class=\"button btn_on\">Go to Hosereel settings</button></a>");
   Value += ("<a href=\"/Hoseguide_Page\"><button class=\"button btn_on\">Go to Hoseguide settings</button></a></p>"); 
...
String Stylecode() {
  String Value =("");
    ...
    Value += ("<body><h1>Boom and Trolley loop Tuner  <a href=\"/Main_Page\"><button class=\"button btn_on\">Go to Main Page</button></a></h1>");
